...
export class RmHeaderActions extends RmHeaderComponent {
  ...
  async checkEssentialElements(): Promise<void> {
    ...
    if (await this.lnkAdmin.isVisible()) {
      await expect(this.lnkAdmin, 'Admin Tab visible').toBeVisible();
    } else {
      console.warn('Admin link not visible for current user.');
    }
  }
  ...
  async openUserMenu(): Promise<void> {
    ...
    await expect(this.page.locator('div.t-NavigationBar-menu.a-Menu:has-text("Sign Out")')).toBeVisible();
  }
  ...
  async goToAdmin(): Promise<void> {
    ...
    expect(count).toBeGreaterThanOrEqual(1);
      ...
      console.warn('Admin link is not visible fof the user.');
    }
  }
}