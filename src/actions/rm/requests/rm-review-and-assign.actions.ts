...
export class RmReviewAndAssignActions extends RmReviewAndAssignComponent {
  ...
  async checkEssentialElements(): Promise<void> {
    ...
    !isPreAward ? await expect(this.btnAssignAvailability, 'Validate Assign Availability button' +
      ' is visible').toBeVisible() : null;
    ...
    !isPreAward ? await expect(this.btnMarkUnfulfilled, 'Validate Mark Unfulfilled button is' +
      ' visible').toBeVisible() : null;
  ...
  async changeRegion(searchText: string): Promise<void> {
    await this.dropRegion.scrollIntoViewIfNeeded();
    await this.dropRegion.click();
    await this.page.keyboard.type(searchText);
    //const items = this.popupRegion.locator('ul > li');
    //const count = await items.count();
    await expect(this.popupRegion.locator('ul > li')).toHaveCount(1, { timeout: 5000 })
    await this.page.keyboard.press('ArrowDown');
    await this.page.keyboard.press('Enter');
    await this.rmCommonActions.waitForLoadState();
  }

  async changeCountry(searchText: string): Promise<void> {
    await this.dropCountry.scrollIntoViewIfNeeded();
    await this.dropCountry.click();
    await this.page.keyboard.type(searchText);
    await expect(this.popupCountry.locator('ul > li')).toHaveCount(1, { timeout: 5000 })
    await this.page.keyboard.press('ArrowDown');
    await this.page.keyboard.press('Enter');
    await this.rmCommonActions.waitForLoadState();
  }
  ...
  async gotoProfileReview(): Promise<void> {
    await this.btnProfileReview.click();
    //await this.page.waitForEvent('load')
    await this.rmCommonActions.waitForLoadState();
  }
}