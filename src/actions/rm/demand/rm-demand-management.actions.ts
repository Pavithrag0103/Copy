...
export class RmDemandManagementActions extends RmDemandManagementComponent {
  ...
  private async validateTab(tabName: string = "NA"): Promise<void> {
    ...
    await expect(this.btnTabAddRequestRegion.nth(index), 'Add Request Region button is visible').toBeVisible();
    await expect(this.btnTabSave.nth(index), 'Save button is visible').toBeVisible();
    await playwrightExtension.softStep('Carry Demand Forward button is visible', async () => {
      await expect(this.btnTabCarryDemandForward.nth(index), 'Carry Demand Forward button is visible').toBeVisible({timeout:1000});
    })
    await playwrightExtension.softStep('Submit Resource Request button is visible', async () => {
      await expect(this.btnTabSubmitResourceRequest.nth(index), 'Submit Resource Request button is visible').toBeVisible({timeout:1000});
    })
    await playwrightExtension.softStep('Tab Overview Demand table is visible', async () => {
      await expect(this.tableTabOverviewDemand.nth(index), 'Tab Overview Demand table in tab is visible').toBeVisible({timeout:1000});
    })
    await playwrightExtension.softStep('Tab Demand table button is visible', async () => {
      await expect(this.tableTabDemand.nth(index), 'Tab Demand table in tab is visible').toBeVisible({timeout:1000});
    })
  }
